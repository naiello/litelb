...
```shell
CC=${ARCH}-linux-musl-gcc cargo build --package litelb --release \
  --target=${ARCH}-unknown-linux-musl \
  --config=target.${ARCH}-unknown-linux-musl.linker=\"${ARCH}-linux-musl-gcc\"
```
The cross-compiled program `target/${ARCH}-unknown-linux-musl/release/litelb` can be
copied to a Linux server or VM and run there.

## License

With the exception of eBPF code, litelb is distributed under the terms
